...
import { extractRunSummary, extractRunCost, formatTimeAgo, formatDuration, classifyPattern } from "./heartbeat-reflection.js";
...
export function heartbeatService(db: Db) {
  ...
  async function buildReflection(agentId: string, companyId: string): Promise<string | null> {
    try {
      ...
      const lines = recentRuns.slice(0, 3).map((run, i) => {
        ...
        const summary = extractRunSummary(run.resultJson as Record<string, unknown> | null);
        const cost = extractRunCost(run.resultJson as Record<string, unknown> | null);
      ...
      const prompt = classifyPattern(recentRuns.map((r) => ({
        status: r.status,
        exitCode: r.exitCode,
        resultJson: r.resultJson as Record<string, unknown> | null,
      })));
  ...
  // extractRunSummary, extractRunCost, formatTimeAgo, formatDuration, classifyPattern
  // are imported from ./heartbeat-reflection.js
